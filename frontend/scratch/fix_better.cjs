...
// 1. Remove the malformed block from 'tree-types' to 'tree-levelorder'
const startMarker = '  "tree-types": {';
const endMarker = '  "tree-levelorder": {';
...
if (startIndex !== -1) {
    ...
    if (endIndex !== -1) {
        ...
        console.log("Removed malformed injections.");
    }
}

// 2. Build the proper objects in JS
const newAdditionsObj = {
  "tree-types": {
    "id": "tree-types",
...
let jsonString = JSON.stringify(newAdditionsObj, null, 2);
// Remove the outer {} to just get the properties
jsonString = jsonString.substring(1, jsonString.length - 1) + ",";

content = content.replace("export const algorithmContent: Record<string, AlgorithmContent> = {", "export const algorithmContent: Record<string, AlgorithmContent> = {" + jsonString);
