...
export const KthSmallestVisualizer: React.FC<Props> = () => {
  const nums = [7, 10, 4, 3, 20, 15];
  const k = 3;

  const [stepIdx, setStepIdx] = useState<number>(-1);
  const [heap, setHeap] = useState<number[]>([]);
  const [actionText, setActionText] = useState<string>('Goal: Find the 3rd smallest element using a Max-Heap of size K = 3');
  ...
  const runSimulation = async () => {
    ...
    for (let i = 0; i < nums.length; i++) {
      ...
      // Insert val into Max-Heap
      currentHeap.push(val);
      currentHeap.sort((a, b) => b - a); // Max Heap invariant sort
      setHeap([...currentHeap]);
      setActionText(`Step ${i + 1}: Inserted ${val} into Max-Heap. Current Heap: [${currentHeap.join(', ')}]`);
      await new Promise(r => setTimeout(r, 900));

      if (currentHeap.length > k) {
        const popped = currentHeap.shift()!;
        setHeap([...currentHeap]);
        setActionText(`Heap size ${currentHeap.length + 1} > K (${k}). Popped largest element ${popped} from root!`);
        await new Promise(r => setTimeout(r, 900));
      }
    }

    setActionText(`Finished processing array! Root of Max-Heap size ${k} is ${currentHeap[0]} (The ${k}rd Smallest Element).`);
    setIsRunning(false);
  };
  ...
  const handleReset = () => {
    setStepIdx(-1);
    setHeap([]);
    setActionText('Ready to start Kth Smallest simulation');
    setIsRunning(false);
  };
    ...
    <div className="w-full flex flex-col gap-6 text-left">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-card/80 border border-border/80 rounded-2xl p-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          >
            <Play className="w-4 h-4 fill-current" /> {isRunning ? 'Running...' : 'Animate Kth Smallest'}
          </button>
          <button
            onClick={handleReset}
            disabled={isRunning}
        ...
        <div className="flex items-center gap-3 font-mono text-xs">
          ...
          <span className="px-2.5 py-1 rounded-lg bg-blue-500/10 text-blue-400 border border-blue-500/30 font-bold">
            Max-Heap Strategy (Size ≤ {k})
          </span>
        </div>
      </div>
      ...
      {/* INPUT STREAM VS MAX-HEAP PIPELINE */}
        ...
        <div className="lg:col-span-5 bg-card/60 border border-border/80 rounded-3xl p-5 flex flex-col justify-between shadow-inner space-y-4">
          ...
          <div className="flex flex-wrap gap-2 justify-center py-4">
            {nums.map((val, idx) => {
              ...
              return (
                <div key={`ks-input-${idx}`} className="flex flex-col items-center gap-1">
                  <motion.div
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            Why Max-Heap for Kth Smallest? A Max-Heap of size K retains the K smallest items seen so far. Its root holds the largest among those K smallest, which is exactly the Kth smallest element!
          </p>
        </div>

        {/* MAX-HEAP OF FIXED SIZE K */}
        <div className="lg:col-span-7 bg-card/60 border border-border/80 rounded-3xl p-5 flex flex-col justify-between shadow-inner space-y-4 border-blue-500/30">
          <div className="flex justify-between items-center">
            <span className="text-[11px] font-bold uppercase tracking-wider text-blue-400 font-sans px-2 py-1 bg-blue-500/10 rounded-lg border border-blue-500/30">
              Active Max-Heap (Max Capacity: K = {k})
            </span>
            <span className="text-xs font-mono text-muted-foreground">Current Root = Kth Smallest Candidate</span>
          </div>
                ...
                return (
                  <motion.div
                    ...
                    className={`
                      ...
                      ${isRootCandidate
                        ? 'bg-blue-500/20 border-blue-500 text-blue-400 scale-105 ring-4 ring-blue-500/10'
                        : 'bg-background border-border text-foreground'}
                    `}
                  >
                    <span>{val}</span>
                    <span className="text-[9px] font-sans text-muted-foreground/60 font-normal absolute -bottom-5">
                      {isRootCandidate ? 'Root (Kth Smallest)' : `[${idx}]`}
                    </span>
                  </motion.div>
                );
              })}
